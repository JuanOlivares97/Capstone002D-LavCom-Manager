...
const tooltipRenderer = ({ title, datum, xKey, yKey }) => ({
    title,
    content: `${dateFormatter.format(datum[xKey])}: ${datum[yKey]}`,
});

function createChart(containerId, data, titleText, seriesOptions, axesOptions, footnoteText) {
    const chartOptions = {
        ...
        footnote: {
            text: footnoteText || "Source: Department for Business, Energy & Industrial Strategy", // Default value if no footnote is passed
        },
        series: seriesOptions.map(({ type, xKey, yKey }) => ({
            type: type || "line", // Default to 'line' if not specified
            xKey,
            yKey,
...
document.addEventListener('DOMContentLoaded', function () {
    createChart(
        "chartForLines",          // ID del contenedor del gráfico
        getData(),          // Función que obtiene los datos
        "Grafico 1", // Título del gráfico
        [                   // Series del gráfico
            { xKey: "date", yKey: "petrol" },
            { xKey: "date", yKey: "diesel" },
        ],
        null                // Opciones por defecto de los ejes
    );

    createChart(
        "chartForPie",          // ID del contenedor del gráfico
        getData(),          // Función que obtiene los datos
        "Grafico 2", // Título del gráfico
        [                   // Series del gráfico
            { xKey: "date", yKey: "petrol" },
            { xKey: "date", yKey: "diesel" },
        ],
