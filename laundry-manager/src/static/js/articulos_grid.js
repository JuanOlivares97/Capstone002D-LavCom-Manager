...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const gridOptions = {
        ...
        columnDefs: [
            { field: "id_articulo", flex: 1 },
            { field: "nombre_articulo", flex: 1 },
            { field: "stock", flex: 1 },
            { field: "id_subgrupo_ropa", flex: 1 },
            { field: "borrado", flex: 1 },
            {
                headerName: "Acciones",
                field: "actions",
                cellRenderer: function (params) {
                    return `
                        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 text-sm rounded mr-1" onclick="editArticulo(${params.data.id_articulo})">Editar</button>
                        <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 text-sm rounded" onclick="deleteArticulo(${params.data.id_articulo})">Borrar</button>
                    `;
                },
                flex: 1
            }
...
function editArticulo(id) {
    alert(`Editando articulo con id ${id}`);
}

function deleteArticulo(id) {
    alert(`Borrando articulo con id ${id}`);
}